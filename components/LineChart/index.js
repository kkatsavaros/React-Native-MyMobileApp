...
import {quickSort} from '../../utils'; // axis y labels
...
const LineChart = ({
  ...
  curve = true,
  //
  lineGradient = true, // gradient under the line
  //
  ...
  //
  showTooltips = true,
  // LinearGradient id="gradientback"
  ...
  //***************************************************** */
  const [yAxisLabels, setYAxisLabels] = useState([]); // [12]
  //
  const x_margin = 50; // Από που θα αρχίζουν οι άξονες x,y
  const y_margin = 50;

  // [12] y axis labels
  useEffect(() => {
    const yKeys = data.map(item => item[y_key]);
    const yAxisData = quickSort(yKeys); // in utils.js
    setYAxisLabels(yAxisData);
  }, []);

  // [9] Πόσο πλάτος θα έχουν τα ticks.
  ...
  // [10] // Σε πιο ύψος θα είναι τα ticks.
    ...
    return (
      <G>
        <Rect
          x={0}
          y={0}
          rx={backgroundBorderRadius} // rounded borders
  ...
  // [8] Είναι οι | κάτω από τον άξονα x.
  ...
  // [9] Είναι τα labels στο άξονα x.
  const render_x_axis_labels = () => {
    const {gap_between_ticks} = calculateWidth();

    const {fontSize, textAnchor, fill, fontWeight, rotation} = x_axis_config; //props

    return data.map((item, index) => {
      const x = x_margin + gap_between_ticks * index; // index: πόσες φορές
  ...
  const render_y_axis_labels = () => {
    ...
    const x = x_margin - 13; /// πόσο πιο δεξιά από τον α΄ξονα υ
  ...
  return (
    <View style={mainContainer}>
      <Svg height="100%" width="100%" style={svgContainer}>
        ...
        {/* [17] Show horizontal lines */}
        {data && data.length > 0 && showHorizontalLines && render_horizontal_lines()}
        {data && data.length > 0 && showVerticalLines && render_vertical_lines()}
